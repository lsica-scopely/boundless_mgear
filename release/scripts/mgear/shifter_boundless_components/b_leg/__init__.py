"""Component Leg MS 2 joints 01 module"""
...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        self.length0 = vector.getDistance(g_apos[0],
                                          g_apos[1])
        self.length1 = vector.getDistance(g_apos[1],
                                          g_apos[2])
        self.length2 = vector.getDistance(g_apos[2],
                                          g_apos[3])

        # FK Controlers -----------------------------------
        # *ms* set npo @ Tpose, to make the fk rotation work best with
        # rot order"yzx"

        self.fk_cns = primitive.addTransformFromPos(
            self.root, self.getName("fk_cns"), g_apos[0])
        tpv = g_apos[0] + ((g_apos[1] - g_apos[0]) * [0, 1, 0])
        t = transform.getTransformLookingAt(
            g_apos[0], tpv, self.normal, "xz", self.negate)
        # *ms* add FK isolation
        self.fk0_npo = primitive.addTransform(
            self.fk_cns, self.getName("fk0_npo"), t)

        t = transform.getTransformLookingAt(
            g_apos[0], g_apos[1], self.normal, "xz", self.negate)
        ...
        t = transform.setMatrixPosition(t, g_apos[1])
        ...
        t = transform.getTransformLookingAt(
            g_apos[1], g_apos[2], self.normal, "xz", self.negate)
        ...
        t = transform.getTransformLookingAt(
            g_apos[2], g_apos[3], self.normal, "z-x", negate=False)

        self.fk2_mtx = primitive.addTransform(
            self.fk1_roll_ctl, self.getName("fk2_mtx"), t)

        # fk2_loc is need to take the effector position + bone1 rotation
        # fk2_npo should get offset rotation from fk2_mtx
        t = transform.getTransformLookingAt(
            g_apos[2], g_apos[1], self.normal, "-xz", self.negate)
    ...
    # tws_ref
        ...
        self.upv1_auv = primitive.addTransform(
            self.ik_ref,
            self.getName("upv1_auv"),
            transform.getTransform(self.ik_ctl))

        self.upv2_auv = primitive.addTransform(
            self.upv1_auv,
            self.getName("upv2_auv"),
            transform.getTransform(self.ik_ctl))
        ...
        self.tws3_npo = primitive.addTransform(
            self.root, self.getName("tws3_npo"), t)

        self.tws3_loc = primitive.addTransform(
            self.tws3_npo, self.getName("tws3_loc"), t)
    ...
    def addAttributes(self):
        ...
        self.blend_att = self.addAnimParam(
            "blend", "Fk/Ik Leg", "double", self.settings["blend"], 0, 1)
